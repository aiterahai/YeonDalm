    ...
    def __init__(self):
        ...
        for i in range(len(self.Encoder_channel) - 1):
            self.Encoder.append(nn.Sequential(*[
                nn.Conv2D(self.Encoder_channel[i], self.Encoder_channel[i], kernel_size=4, stride=2, padding=1,
                          groups=self.Encoder_channel[i]),
        ...
        for i in range(len(self.Decoder_inchannel)):
            self.Decoder.append(nn.Sequential(*[
                nn.Conv2D(self.Decoder_inchannel[i], self.Decoder_inchannel[i], kernel_size=3, stride=1, padding=1,
                          groups=self.Decoder_inchannel[i]),
...
class BuildFaceSwap(nn.Layer):
    def __init__(self, opt=None):
        ...
        self.Encoder_channel = [3, 64 // encoder_scale, 128 // encoder_scale, 256 // encoder_scale,
                                512 // encoder_scale, 1024 // encoder_scale]
        ...
        self.Decoder_inchannel = [1024 // encoder_scale, 1024 // encoder_scale, 512 // encoder_scale,
                                  256 // encoder_scale]
        self.Decoder_outchannel = [512 // encoder_scale, 256 // encoder_scale, 128 // encoder_scale,
                                   64 // encoder_scale]
        ...
        mask_channel = [1024 // encoder_scale, 256 // encoder_scale, 128 // encoder_scale, 64 // encoder_scale,
                        16 // encoder_scale, 4 // encoder_scale]
        mask = []
        for i in range(len(mask_channel) - 1):
            mask.append(nn.Sequential(
                nn.Upsample(scale_factor=2.0, align_corners=True, mode='bilinear'),
